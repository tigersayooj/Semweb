 ...
 *
 * $Id: foaf.js,v 1.3 2002/07/23 15:46:04 ccslrd Exp $
 */
...
gGeneratorAgent = 'http://www.ldodds.com/foaf/foaf-a-matic';
gErrorReportsTo = 'mailto:kdevkp77@gmail.com';
...
function PersonSerializer(p, merging)
{
    ...
    this.documentDescription = '<foaf:PersonalProfileDocument rdf:about=\"\">\n' +
...
function getFOAF()
{
    return (this.merging ? this.makePerson() : this.top + this.documentDescription + this.makePerson() + '\n' + this.tail);
}
...
function makeOpeningTag(prefix, localname, id)
{
    if (id)
    {
        return '<' + prefix + ':' + localname + ' rdf:ID="' + id + '">';
    }
    else
    {
        return '<' + prefix + ':' + localname + '>';
